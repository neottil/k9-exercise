...
| `POST` | `/api/admin/gc-images` | garbage collection orfani (Bearer `NOTIFY_API_KEY`) |
...
curl -s -X POST http://localhost:3001/api/admin/gc-images \
  -H "Authorization: Bearer <NOTIFY_API_KEY dal .env>"
```
...
- `MINIO_ENDPOINT`, `MINIO_PORT`, `MINIO_USE_SSL`, `MINIO_BUCKET` (valori in chiaro)
- `MINIO_ACCESS_KEY` / `MINIO_SECRET_KEY` (dal secret)
